# Project Guidelines — GraphRAG Polizei Knowledge Graph Demo

## Overview
Landing page for a knowledge graph & GraphRAG demo with an interactive 3D police knowledge graph ("Operation Hydra"). All UI text is **German**. Domain: law enforcement, legal references (StPO, StGB, BtMG, GwG, BSIG, NIS2, DSGVO). Demo only — not affiliated with any vendor.

Deployed at: `https://<user>.github.io/graph-insurance/` (base path `/graph-insurance/`)
...
├── vite.config.ts                # base: '/graph-insurance/', plugins, path alias
...
## Vite Configuration
- **Base**: `/graph-insurance/` (subpath deployment)
